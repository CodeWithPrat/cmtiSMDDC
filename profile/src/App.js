...
import { Sun, Moon, Menu, X, ChevronRight, Home, Briefcase, Users, Info, Pyramid, Mail, ArrowRight, Phone, Images , MapPin, ArrowRightCircle } from 'lucide-react';
...
const HomePage = () => {
  ...
  return (
    <div className="w-full overflow-hidden bg-slate-900 text-white">
      {/* Hero Section */}
      <section className="relative min-h-screen w-full mt-[-5rem] ">
        {/* Dynamic background */}
          ...
          >
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 w-full">
              {[
                ...
                >
                  <div className={`absolute inset-0 bg-gradient-to-r ${feature.color === "blue" ? "from-blue-500/20 to-blue-700/20" :
                  ...
                  <div className="h-full relative p-8 rounded-2xl bg-white/5 backdrop-blur-sm border border-white/10 hover:border-white/20 transition-all duration-300 group-hover:transform group-hover:translate-y-[-4px]">
                    <div className={`inline-flex items-center justify-center p-3 mb-6 rounded-xl ${feature.color === "blue" ? "bg-blue-500/20 text-blue-400" :
                    ...
                    <div className={`mt-6 inline-flex items-center text-sm font-medium ${feature.color === "blue" ? "text-blue-400" :
...
const NavLink = ({ to, children, icon: Icon, onClick }) => {
  ...
  return (
    <Link
      to={to}
      onClick={onClick}
      className={`flex items-center space-x-2 px-6 py-2.5 rounded-full transition-all duration-300 group ${isActive
...
const App = () => {
  ...
  return (
    <Router>
      <ScrollToTop />
      <div className={`min-h-screen bg-white ${isDark ? 'dark:bg-gray-900 dark:text-gray-100' : 'text-gray-900'} transition-colors duration-300`}>
        {/* Enhanced Navbar with glass effect */}
        <nav className={`fixed w-full top-0 z-40 transition-all duration-300 ${isScrolled
